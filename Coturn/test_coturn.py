...
def main():
    parser = argparse.ArgumentParser(description='Test Coturn TURN/STUN server connectivity')
    parser.add_argument('--host', default='localhost', help='TURN/STUN server host (default: localhost)')
    parser.add_argument('--port', type=int, default=3478, help='TURN/STUN server port (default: 3478)')
    parser.add_argument('--username', default='nexy', help='TURN username (default: nexy)')
    parser.add_argument('--password', default='nexy_turn_password', help='TURN password')
    parser.add_argument('--realm', default='nexy.local', help='TURN realm (default: nexy.local)')
    ...
    # Test STUN
    stun_result = test_stun_server(args.host, args.port)
    
    # Test TURN
    turn_result = test_turn_authentication(args.host, args.port, args.username, args.password, args.realm)
    ...
    if stun_result and turn_result:
        ...
        print("\nNext steps:")
        print("1. Ensure Nexy server is configured with this TURN server")
        print("2. Build and install the Android client")
        print("3. Test calls between devices on different networks")
        return 0
    else:
        print("\n✗ Some tests failed. Please check:")
        print("1. Coturn container is running: docker ps")
        print("2. Coturn logs: docker logs nexy_coturn")
